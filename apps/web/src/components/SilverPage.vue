...
async function loadStatus() {
  try {
    state.value = await apiGet('/api/jobs/silver/status')
...
async function loadReport(date) {
  try {
    const r = await apiGet('/api/jobs/silver/report?date=' + encodeURIComponent(date))
...
async function run() {
  ...
  err.value = ''
  try {
    const r = await apiPost('/api/jobs/silver/run', { push: pushOn.value })
  ...
  <div class="silver-body">
    <div class="modebar">
      <span class="chip-mode" :class="state && state.llm ? 'ok' : 'warn'">
        <i class="dot2"></i>
        {{ state && state.llm ? 'DeepSeek 已配置 · 可生成' : 'DeepSeek 未配置（server/.env）' }}
      </span>
      <span class="chip-mode" :class="state && state.sc ? 'ok' : 'warn'">
        <i class="dot2"></i>{{ state && state.sc ? 'Server酱 已配置' : 'Server酱 未配置' }}
      </span>
      <span class="modebar-hint">调度：每日 17:00（GH Actions）· 也可手动生成</span>
    </div>

    <section class="rmodule">
      <div class="rm-head">
        <div class="rm-titles">
          <div class="rm-kicker">SILVER CARE MEDIA · M4 已接入</div>
          <h3 class="rm-title">银发康养爆款日报（今日头条 / 小红书）</h3>
        </div>
        <span class="rm-demo" :class="{ warn: !state || !state.llm }">
          <BaseIcon name="check" :size="12" /> 17:00 自动 · LLM 拆解+改写
        </span>
      </div>

      <div class="daily-body">
        <div class="daily-status">
          <div class="ds-item"><span>关键词</span><b>{{ (state && state.keywords ? state.keywords : ['银发经济', '康养', '养老产业']).join(' · ') }}</b></div>
          <div class="ds-item">
            <span>最近运行</span>
            <b v-if="state && state.last_run">
              {{ state.last_run.date }} · 真实 {{ state.last_run.real }} / 模拟 {{ state.last_run.sim }} ·
        ...
        <div class="up-actions">
          <label class="chk"><input type="checkbox" v-model="pushOn" /><span>生成后推送个人微信（摘要+改写稿）</span></label>
          <button class="btn-primary" :disabled="busy || !state || !state.llm" @click="run">
            <BaseIcon name="radar" :size="14" /> 立即生成今日日报
          </button>
          <span v-if="busy" class="up-loading"><i class="spin"></i> 抓热榜 + LLM 拆解改写中（约 1 分钟）…</span>
        </div>
        ...
        <div v-if="state && state.last_run" class="top3-strip">
          ...
          <span v-for="(t, i) in state.last_run.top3" :key="i" class="tt-item">
            {{ i + 1 }}.《{{ t.title }}》<em>{{ t.metric }}{{ t.sim ? ' · AI模拟' : '' }}</em>
          </span>
        </div>
      </div>
    </section>
    ...
    <section v-if="reportMd" class="rmodule report-panel">
      <div class="rm-head">
        ...
        <span class="rm-demo">文件：data/media/reports/silver_{{ reportDate }}.md</span>
      </div>
